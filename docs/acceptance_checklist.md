...
- ⚙️ Алерт на AI-бюджет >90% ✅; глубина очереди Celery видна в `/health/deep`, авто-алерт >50 — ⏳
...
**35.12 Mini App и приёмка.** ✅ SPA (роутер + компоненты + 11 экранов, тема Telegram), отдача статики, синтаксис-проверка в CI. ✅ ~150 автотестов, `ruff`+`pytest`(Postgres+Redis)+node-check, авто-деплой на зелёных тестах. ⏳ Нагрузочный тест и ежедневный отчёт руководителю.
